// Embedded homepage HTML for Vercel - no file system dependency
...
<div class="logo"><a href="index.html">P hub</a></div>
...
async function loadUploadedVideos(){
  ...
  try{
    ...
    if(allVideos.length===0){
      var slug=query?String(query).toLowerCase().replace(/\s+/g,"-").replace(/[^a-z0-9-]/g,"")||"videos":"";var pornmdUrl=slug?"https://www.pornmd.com/search/a/"+encodeURIComponent(slug):"";videoList.innerHTML=query?'<div class="no-results-box"><p>No videos found for "'+escapeHtml(query)+'".</p><p style="font-size:.85rem;color:var(--text-muted);margin-top:8px">Add BROWSERLESS_WS_URL in Vercel to show PornMD results here.</p>'+(pornmdUrl?'<a href="'+pornmdUrl+'" target="_blank" rel="noopener" class="btn btn-primary" style="display:inline-block;margin-top:12px">Search on PornMD →</a>':'')+'<p style="margin-top:16px"><a href="index.html">Show all</a></p></div>':"<p>No videos yet. <a href='upload.html'>Upload the first one</a>.</p>";
      return;
    }
